# AWS Elastic Container Service (ECS) - Task Definition JSON Terraform module

Terraform module that will create the Task Definition JSON and container log group to be consumed by the Task Definition module.

## Pre-requirements
***
- This module creates a well-formed JSON document that specifies all the required resources to be granted to a container in ECS
- For more information please see: https://docs.aws.amazon.com/AmazonECS/latest/developerguide/create-task-definition.html
...
module "td" {
  source = "../../td-json"
  container_image = module.ecr.ecr_url
  container_name = local.ecs-service-name
  essential = true
  ...
  mount_points = [
    ...
    {
      containerPath = "/usr/local/tomcat/conf/service/"
      sourceVolume = "service-configs"
      readOnly = true
    }
  ]
  ...
  log_configuration = {
    logDriver = "awslogs"
    options = {
      "awslogs-group" = "${local.ecs-service-name}-LogGroup"
      "awslogs-region" = local.ecs-service-region
      "awslogs-stream-prefix" = "${local.ecs-service-name}-LogGroup-stream"
    }
    secretOptions = null
  }
}
